...
const App = () => {
  ...
  const startScan = () => {
    if (!isScanning) {
      BleManager.scan([], 3, true).then((results: Array<any>) => {
  ...
  const retrieveConnected = () => {
    BleManager.getConnectedPeripherals([]).then((results: Array<any>) => {
      if (results.length == 0) {
    ...
    BleManager.connect(peripheral.id)
      .then(() => {
        ...
        setTimeout(() => {
          ...
          BleManager.retrieveServices(peripheral.id).then((peripheralData: any) => {
            // console.log('Retrieved peripheral services', peripheralData);
            console.log("Retrieved peripheral services");

            BleManager.readRSSI(peripheral.id).then((rssi: number) => {
      ...
      console.warn("Connection error when connecting to: " + peripheral.id, error);
    });
  };
